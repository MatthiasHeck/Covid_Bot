...
townURL = "https://maps.ct.gov/arcgis/rest/services/CT_DPH_COVID_19_PROD_Layers/FeatureServer/0/query?f=json&where=1%3D1&returnGeometry=false&spatialRel=esriSpatialRelIntersects&outFields=*&resultOffset=0&resultRecordCount=*"
...
async def write_county_data(data):
    filename = "D:\\Projects\\Python\\Covid_Bot\\DATA_OLD\\" + str(data['Tolland']['DateLastUpdated']) + "_COUNTY_DATA.TXT"
    if os.path.exists(filename):
        return
    else:
        file_obj = open(filename, 'w')
...
async def populate_town_data():
    r = requests.get(url=townURL)
    rawdata = r.json()
    data = rawdata['features']

    for i in range(0, data.__len__()):
        dataByTown[data[i]['attributes']['TOWN']] = data[i]['attributes']

    # await write_town_data(dataByCounty)
    return


async def write_town_data():
    return
...
@bot.command(help="Use !town to view syntax")
async def town(ctx, *arg):
    """
    0 arg - return syntax
        !town <county> - list towns in this county
        !town <town> - list information for this town
    1 arg -
        !town <town> - returns COUNTY, TOWN, PLAN_ORG, ConfirmedCases, Deaths

        if no results then

        !town <county> - returns list of towns in county
    """

    if 2 < arg.__len__() == 0:
        response = '```\n' \
                   '!town county <county> - Returns a list of Towns for this County\n' \
                   '!town <town> - Returns COVID-19 information about this Town\n' \
                   '```'
        await ctx.send(response)
        return

    try:
        if arg[0].lower() != 'county':
            if arg.__len__() == 2:
                town = arg[0].title() + " " + arg[1].title()
            else:
                town = arg[0].title()

            if town in dataByTown.keys():
                d = dataByTown[town]
                response = '```' \
                           'Town: %s\n' \
                           'County: %s\n' \
                           'Confirmed Cases: %s\n' \
                           'Deaths: %s\n' \
                           '```' % (d['TOWN'], d['COUNTY'], d['ConfirmedCases'], d['Deaths'])
            ...
            cty = arg[1].title()
            d = []
            for key in dataByTown.keys():
                if dataByTown[key]['COUNTY'] == cty:
                    d.append(dataByTown[key]['TOWN'])
